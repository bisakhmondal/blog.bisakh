...
lastmod: '2022-01-18'
tags:
  ['vault', 'hcp', 'hashicorp', 'jwt', 'authentication', 'open-source', 'Apache', 'APISIX', 'Lua']
draft: false
layout: PostSimple
images: ['/static/images/vault/vault.png']
summary: 'This article is regarding integration of HashiCorp Vault as an alternative storage backend with Apache APISIX JWT-Auth plugin'
---
...
<div className="flex flex-wrap justify-center    -mx-2 overflow-hidden xl:-mx-2">
<Image src="/static/images/vault/vault-triangle.png" height={296} width={500}/>
</div>
<h6 className="text-center py-4 text-xs italic"> Source: [Vault](https://learn.hashicorp.com/tutorials/vault/getting-started-intro?in=vault/getting-started) </h6>
...
Internally, APISIX communicates with vault server [KV secret engine](https://www.vaultproject.io/docs/secrets/kv/kv-v1) v1 [HTTP APIs](https://www.vaultproject.io/api/secret/kv/kv-v1). As most enterprise solution prefers to stick with KV Secrets Engine - Version 1 in their production environment, during the initial phase of APISIX-Vault support we have gone with version 1 only. In later releases, we will add the support of K/V version 2.

The main idea of using vault, instead of the APISIX etcd backend is the security concern in a low trust environment. We, the APISIX developers, understand your priorities seriously. That's why we recommend using vault access tokens that are short scoped and can grant APISIX server with limited access.
...
Here you have multiple options, feel free to choose between docker, precompiled binary or building from source. As to communicate with the vault server, you need a vault CLI client, I would prefer going with precompiled binary instead of the docker approach. Anyway, it's totally up to you (feel free to consult official [installation docs](https://www.vaultproject.io/docs/install)). To spin up a development server, please run
...
This article is regarding using vault in jwt-auth plugin perspective. So, for an APISIX consumer (if you are unfamiliar with consumers in the APISIX ecosystem, please read the [doc](https://apisix.apache.org/docs/apisix/architecture-design/consumer)) with username `jack` the jwt-auth plugin looks up (if enabled with vault configuration) for secret/s at `<vault.prefix inside config.yaml>/consumer/<consumer.username>/jwt-auth` into vault kv storage. In this context, if you are assigning `kv/apisix` namespace (vault path) as `vault.prefix` inside config.yaml for all apisix related data retrieval, we suggest you to create a policy for path `kv/apisix/consumer/*`. The extra asterisk (`*`) at the end ensure the policy allows read for any path that has a `kv/apisix/consumer` prefix.
...
APISIX has a consumer-level abstraction that goes side by side with authentication scenarios. To enable authentication for any APISIX route, a consumer is needed with a suitable configuration for that specific type of authentication service. Then only APISIX can forward the request to the upstream URI by successfully performing authentication wrt. the consumer configuration. APISIX consumer has two fields - one is `username` (mandatory) to identify one consumer from the others and another is `plugins` that holds the consumer specific plugin configurations.

Here, in this article, we will create a consumer with `jwt-auth` plugin. It performs [JWT authentication](https://jwt.io/introduction) for the respective route/s or service/s.
...
> **Note**: For the early version of this integration support, the plugin expects the key name of secrets stored into the vault path is among [ `secret`, `public_key`, `private_key` ] to successfully use the key. In future releases, we are going to add the support of referencing custom-named keys.
...
2. RS256 RSA keypairs, both public and private keys are stored in the vault.
   ...
   The plugin looks up for `public_key` and `private_key` keys inside vault kv path (`<vault.prefix from conf.yaml>/consumer/jim/jwt-auth`) for username `jim` mentioned inside plugin vault configuration. If not found, authentication fails.
...
✅ [PR#5745](https://github.com/apache/apisix/pull/5745) feat(vault): vault lua module, integration with jwt-auth authentication plugin
